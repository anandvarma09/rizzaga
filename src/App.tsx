...
function App() {
  const [currentView, setCurrentView] = useState<'wall' | 'explore' | 'contacts' | 'chats' | 'profile' | 'about'>('wall');
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a0033, #0f172a)', color: 'white', paddingBottom: '90px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ paddingTop: '40px', maxWidth: '620px', margin: '0 auto', padding: '15px' }}>
        ...
        {currentView === 'about' && (
          <div style={{ padding: '40px 20px', textAlign: 'center' }}>
            <h2 style={{ color: '#f59e0b' }}>About Rizzaga</h2>
            <p style={{ fontSize: '18px', lineHeight: '1.8', margin: '20px 0' }}>
              Rizzaga is a privacy-first social network built with real AES-GCM encryption, 24-word seed phrases, and P2P torrent sharing. 
              No servers store your data — everything stays on your device. 
              Mission: Give users full control over their data and communication.
            </p>
          </div>
        )}
      ...
      <nav style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: 'rgba(15,23,42,0.95)', display: 'flex', justifyContent: 'space-around', padding: '16px 0', borderTop: '1px solid #334155', backdropFilter: 'blur(16px)' }}>
        ...
        <button onClick={() => setCurrentView('about')} style={{ fontSize: '32px' }}>ℹ️</button>
      </nav>
