...
import { pubsub } from '@helia/pubsub';
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a0033, #0f172a)', color: 'white', paddingBottom: '90px', fontFamily: 'system-ui, -apple-system, sans-serif' }}>
      <div style={{ paddingTop: '40px', maxWidth: '620px', margin: '0 auto', padding: '15px' }}>
        {/* Wall, Contacts, Chats, Profile sections remain the same as your last version */}
    ...
    {followedUsers.length > 0 ? followedUsers.map(user => (
      <div key={user} style={{ background: 'rgba(30,41,55,0.95)', padding: '20px', margin: '12px 0', borderRadius: '18px', border: '1px solid #6366f1', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          <span>{user}</span>
          <span style={{ color: onlineUsers.includes(user) ? '#22c55e' : '#ef4444', fontSize: '20px' }}>
            ●
          </span>
        </div>
        <button onClick={() => directMessage(user)} style={{ padding: '8px 20px', background: '#c084fc', color: 'white', border: 'none', borderRadius: '12px', fontWeight: 'bold' }}>💬 Chat</button>
      </div>
    )) : <p style={{ color: '#94a3b8' }}>No connections yet. Connect using Account ID above.</p>}
  </div>
)}

        {/* Other views remain the same as your previous code */}
      </div>
