...
function App() {
  ...
  // Full BIP39 style word list (4000+ words simulation)
  ...
  const correctText = (text: string) => {
    return text
      ...
      .replace(/\b(ur|you're)\b/gi, "you are");
  };
  ...
  const messageUser = () => {
    alert("💬 Opening chat (coming soon with E2EE)");
  };

  const shareMagnet = (id: number) => {
    const magnet = `magnet:?xt=urn:btih:${Date.now().toString(36)}${id}&dn=Rizzaga_Post_${id}`;
    alert(`Magnet Link (Torrent Style):\n\n${magnet}\n\nShare this for P2P encrypted transfer.`);
  };
    ...
    return (
      <div style={{ minHeight: '100vh', background: 'linear-gradient(135deg, #1a0033, #000)', color: 'white', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
        <div style={{ maxWidth: '420px', width: '100%', background: 'rgba(26,26,26,0.95)', padding: '40px', borderRadius: '24px', textAlign: 'center', border: '1px solid #c026d3' }}>
          ...
          <p style={{ color: '#eab308', marginBottom: '30px' }}>Private • Encrypted • Blockchain + Torrent</p>
          ...
          <textarea value={inputPhrase} onChange={(e) => setInputPhrase(e.target.value)} placeholder="Paste your 12-word keyphrase to recover..." style={{ width: '100%', height: '130px', background: '#111', border: '1px solid #444', borderRadius: '16px', padding: '16px', color: 'white', marginBottom: '20px' }} />
      ...
      <div style={{ paddingTop: '90px', maxWidth: '620px', margin: '0 auto', padding: '15px' }}>
            ...
            {myPosts.map((post) => (
              <div key={post.id} style={{ background: '#1f1f1f', padding: '25px', borderRadius: '20px', marginBottom: '20px', border: '1px solid #eab308' }}>
                ...
                <div style={{ display: 'flex', gap: '20px', marginTop: '15px' }}>
                  <button onClick={() => likePost(post.id)} style={{ background: 'none', border: 'none', color: '#eab308', fontSize: '22px' }}>⭐ {post.likes || 0}</button>
                  <button onClick={messageUser} style={{ background: 'none', border: 'none', color: '#22d3ee', fontSize: '22px' }}>💬</button>
                  <button onClick={() => shareMagnet(post.id)} style={{ background: 'none', border: 'none', color: '#a855f7', fontSize: '22px' }}>🔗</button>
                </div>
              </div>
            ))}
          </>
        ...
        {currentView === 'explore' && (
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            <h2 style={{ color: '#22d3ee', fontSize: '32px' }}>🔥 Explore Public Posts</h2>
            <p style={{ color: '#888', marginTop: '20px' }}>TikTok / Reels style public feed with sponsored crypto content coming soon</p>
          </div>
        )}

        {currentView === 'contacts' && <div style={{ padding: '80px 20px', textAlign: 'center', fontSize: '28px', color: '#eab308' }}>👥 Your Network & Connections</div>}
        {currentView === 'chats' && <div style={{ padding: '80px 20px', textAlign: 'center', fontSize: '28px', color: '#a855f7' }}>💬 Encrypted Chats</div>}

        {currentView === 'profile' && (
          <div style={{ padding: '40px 20px', textAlign: 'center' }}>
            <h2 style={{ color: '#c026d3', marginBottom: '30px' }}>👤 Profile</h2>
            <button onClick={() => alert("Your Keyphrase (Encrypted Account):\n\n" + seedPhrase)} style={{ margin: '15px 0', padding: '16px 40px', background: '#eab308', color: 'black', borderRadius: '12px', fontWeight: 'bold' }}>
              View Keyphrase
            </button>
